 ...
 */
@Service
public class Jedis2Visitor {

    private final Logger logger = LoggerFactory.getLogger(Jedis2Visitor.class);

    /**
     * 字段名与"Redis1Configuration"自动注入的名称相同,则读取对应配置的数据，如:redisPool2
     */
    @Autowired
    ...
    private Jedis getClient() {
        try {
            return this.redisPool2.getResource();
        } catch (Exception e) {
