 ...
 * RabbitMQ 监听器 Cart
 */
@Configuration("rabbitMQListenerCartConsumerConfiguration")
public class RabbitMQListenerCartConsumerConfiguration {

    //坑点：cartConnectionFactory 只能通过@Autowired注入，否则在多ConnectionFactory环境下通过参数注入默认是第一个注入的连接
    @Autowired
    @Qualifier("cartConsumerConnectionFactory")
     ...
     */
    @Bean
    public SimpleMessageListenerContainer cartContainer(ChannelAwareMessageListener listener) {
        ...
        container.setQueueNames(RabbitMqConfig.QUEUE_CART_ADD_NAME, RabbitMqConfig.QUEUE_CART_DELETE_NAME);
     ...
     */
    /*@Bean
    public SimpleRabbitListenerContainerFactory cartContainer() {
