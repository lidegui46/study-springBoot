 ...
 * </pre>
 */
@Configuration("RabbitMQQueueBindingCreateOrderProducerConfiguration")
public class RabbitMQQueueBindingCreateOrderProducerConfiguration {
    @Autowired
    ...
    /**
     * 创建 RabbitMQ Queue 【create】
     *
     * @param orderProducerRabbitAdmin 已申明订单RabbitAdmin，此RabbitAdmin绑定了对应的ConnectionFactory
     * @return 已绑定指定申明（connectionFactory）的队列
     */
    @Bean("createOrderProducerQueue")
    public Queue createOrderQueue(RabbitAdmin orderProducerRabbitAdmin) {
        return rabbitMQQueue.createQueue(orderProducerRabbitAdmin, RabbitMqConfig.QUEUE_ORDER_CREATE_NAME);
    }
     ...
     * @param createOrderProducerQueue      已申请的支付队列
     * @return 已绑定指定申明（connectionFactory）的绑定
     */
    @Bean("bindingPayOrder")
    public Binding bindingPayOrder(RabbitAdmin orderProducerRabbitAdmin, TopicExchange orderProducerExchange, Queue createOrderProducerQueue) {
        return rabbitMQTopicExchange.createBinding(orderProducerRabbitAdmin, orderProducerExchange, createOrderProducerQueue, RabbitMqConfig.ROUTING_ORDER_CREATE_KEY);
    }
}
